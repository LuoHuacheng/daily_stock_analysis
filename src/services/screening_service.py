...
DSA_SCREENING_DAILY_FETCH_MAX_WORKERS = 4
DSA_SCREENING_DAILY_CALL_TIMEOUT_SECONDS = 8
DSA_SCREENING_DAILY_HISTORY_CACHE_TTL_HOURS = 24
# Screening's MA-bullish hard filter compares MA5, MA20, and MA60. A source
# with fewer than 60 bars cannot evaluate that filter and must not win merely
# because it returned a non-empty frame.
DSA_SCREENING_DAILY_MIN_BARS_FOR_MA_FILTERS = 60
_DSA_DAILY_HISTORY_TIMEOUT_SLOTS = threading.BoundedSemaphore(DSA_SCREENING_DAILY_FETCH_MAX_WORKERS)
        ...
        *,
        ...
        try:
            ...
            if normalized is not None and not normalized.empty:
                logger.info(
                    "Screening DSA daily history for %s has %d bars, below the %d needed for MA filters; "
                    "falling back to Screening source %s",
                    code,
                    len(normalized),
                    DSA_SCREENING_DAILY_MIN_BARS_FOR_MA_FILTERS,
                    source,
                )
        except Exception as exc:
            logger.warning(
        ...
        for fallback_source in ("sina", "akshare"):
            ...
            try:
                ...
                logger.info(
                    "Screening daily fallback source %s failed for short primary history %s: %s",
                    fallback_source,
                    code,
                    exc,
                )
            ...
            if _has_minimum_daily_history(fallback):
                logger.info(
                    "Screening daily fallback source %s supplied %d bars for %s after short %s history",
                    fallback_source,
                    len(fallback),
                    code,
...
def _get_dsa_daily_history_with_timeout(stock_code: str, *, lookback_days: int) -> Tuple[Any, str]:
    ...
    """
    timeout_seconds = DSA_SCREENING_DAILY_CALL_TIMEOUT_SECONDS
...
def _has_minimum_daily_history(raw_df: Any) -> bool:
    ...
    if not hasattr(raw_df, "columns"):
        return True
    try:
        return len(raw_df) >= DSA_SCREENING_DAILY_MIN_BARS_FOR_MA_FILTERS
    except TypeError:
        return True
