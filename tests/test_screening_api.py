...
class ScreeningOpportunitiesApiTestCase(unittest.TestCase):
    ...
    def test_daily_history_fetcher_forwards_cache_parameters_to_native_fallback(self) -> None:
        from src.services.screening import daily as daily_module

        original_daily_fetch = MagicMock(return_value="native-fallback")
        with (
            patch.object(daily_module, "fetch_daily_history", original_daily_fetch),
            patch.object(screening_service, "_get_dsa_daily_history_with_timeout", side_effect=RuntimeError("DSA unavailable")),
        ):
            ...
            result = fetcher(
                ...
                source="auto",
                retries=3,
                cache_dir="/tmp/screening-daily-cache",
    ...
    def test_daily_history_fetcher_retries_native_sources_for_short_dsa_history(self) -> None:
        from src.services.screening import daily as daily_module
        ...
        with (
            ...
            patch.object(
                screening_service,
                "_get_dsa_daily_history_with_timeout",
                return_value=(pd.DataFrame({"close": range(1, 30)}), "EfinanceFetcher"),
            ),
        ):
            fetcher = screening_service._build_screening_dsa_daily_history_fetcher()
            self.assertIsNotNone(fetcher)
            result = fetcher("601127", source="auto")

        self.assertIs(result, complete_history)
        self.assertEqual(
            original_daily_fetch.call_args_list,
            [
                call("601127", lookback_days=120, source="auto", retries=2, cache_dir=None, cache_ttl_seconds=None),
                call("601127", lookback_days=120, source="sina", retries=2, cache_dir=None, cache_ttl_seconds=None),
            ],
        )
